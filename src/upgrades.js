// ===========================================================================
//  upgrades.js — "THE DEEP" (the upgrade screen) and the MAIN MENU
// ---------------------------------------------------------------------------
...
(function (global) {
  ...
  // ===========================================================================
  //  THE DEEP — the living underwater backdrop, shared by both screens
  // ===========================================================================
  const Deep = {
    built: false, t: 0,
  ...
  const Upgrades = {
    ...
    open() {
      ...
      this.hover = null; this.hoverW = null; this.drag = null;
    ...
    update(dt, t) {
      ...
      if (Input.hit && (Input.hit('ArrowRight') || Input.hit('KeyE'))) this.setTab(this.tab + 1);
      if (Input.hit && (Input.hit('ArrowLeft') || Input.hit('KeyQ'))) this.setTab(this.tab - 1);
      if (Input.down && Input.down('ArrowDown')) this.scrollTo += 260 * dt;
      if (Input.down && Input.down('ArrowUp')) this.scrollTo -= 260 * dt;
      ...
      if (this.hover && (!this.effectCache.id || this.effectCache.id !== this.hover.id)) {
        this.effectCache.id = this.hover.id;
        this.effectCache.rows = this.hover.weapon ? null : nodeEffect(tree, this.hover);
      }
      if (!this.hover) this.effectCache.id = null;
    ...
    setTab(i) {
      ...
      this.tab = n; this.scroll = this.scrollTo = 0; this.hover = null;
    ...
    render(ctx, t) {
      ...
      const chain = this.hover ? this.chainOf(this.hover) : null;
      ...
      for (const n of nodes) {
        ...
        for (const rid of n.req) {
          ...
          const lit = chain && (chain.has(rid) || this.hover === n) && (chain.has(rid) || chain.has(n.id) || this.hover === n);
    ...
    drawCardIdle(ctx, tree, X, y, Wd, T) {
      pixelTextOutlined(ctx, 'THE DEEP', X, y, 12, '#ffe48f', '#2a1d08'); y += 14;
      for (const l of wrapText(ctx, 'He is sinking and he is out of air. Every upgrade is a bubble. Hover one to weigh it up, click to let him tear it open.', Wd, 6)) {
  ...
  const MainMenu = {
    ...
    items() {
      ...
      list.push({ id: 'deep', label: 'THE DEEP' });
    ...
    drawRunPanel(ctx, T) {
      ...
      if (running && st) {
        ...
        for (const l of wrapText(ctx, 'A manatee with a gun-toting otter on her back against an entire fishing village. Salvage the wrecks, spend it in THE DEEP.', w - 20, 6)) {
          pixelText(ctx, l, X, Y, 6, '#cfe6f2'); Y += 9;
        }
        Y += 2;
      }
