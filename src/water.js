...
class Ocean {
  ...
  buildPalettes() {
    ...
    this.waterRamp = ['#3ad3b4', '#2ac7b1', '#1db5ac', '#16a3a4', '#11929b', '#0f818f', '#0e7181', '#0e6273',
      '#0d5464', '#0c4755', '#0b3b47', '#0a3039', '#08262e', '#071d24', '#05161b', '#040f13'].map(hx);
    ...
    for (let b = 0; b < 16; b++) {
      const d = b / 15;
      this.visLUT[b] = Math.round(clamp((0.86 - d) / 0.52, 0, 1) * 5) / 5 * 0.62;
    ...
    for (let b = 0; b < 16; b++) {
      const d = b / 15;
      const ar = 0.78 - d * 0.55, ag = 0.96 - d * 0.42, ab = 1.06 - d * 0.20;
