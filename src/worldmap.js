...
(function (global) {
  ...
  const NOTES = [
    ...
    { l: ['TIDE TURNS AT DUSK'], x: 96, y: 234, c: 'green' },
  ...
  function nearDetail(ctx, mask, dOut, keepOut) {
      ...
      for (let k = 0; k < 26 && !ok; k++) {
        ...
        for (let c = 0; c <= wide; c += 6) {
          for (let r = 0; r < n.l.length; r++) {
            ...
            if (xi < IN.x0 + 8 || xi > IN.x1 - 8 || yi < IN.y0 + 10 || yi > IN.y1 - 12 || mask[yi * MAP_W + xi]) { ok = false; break; }
          }
          if (!ok) break;
        }
        if (ok) ny = ty;
    ...
    pinnedNote(ctx, 92 * SC, 234 * SC, 92, 30, ['HE IS ALIVE.', 'THEY TOOK HIM EAST.'], 0.06);
